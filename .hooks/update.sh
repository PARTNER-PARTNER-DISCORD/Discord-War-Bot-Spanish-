...
if grep -q " git@github.com:" .git/config; then
  ...
  ssh-agent bash -c "ssh-add ~/.ssh/sb_id_rsa_nopass && git fetch --all && git reset --hard origin/master && git submodule update && npm i --production && npm remove youtube-dl"; kill $SSH_AGENT_PID
else
